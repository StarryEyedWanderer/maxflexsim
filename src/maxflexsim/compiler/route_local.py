...
def route(graph: Graph, fabric: Fabric, default_capacity: float = 64.0) -> RoutingResult:
    link_usage: dict[tuple[tuple[int, int], tuple[int, int]], Link] = {}
    for edge in graph.edges:
        ...
        for i in range(len(path) - 1):
            hop = (path[i], path[i + 1])
            link = link_usage.setdefault(hop, Link(capacity_bits_per_cycle=default_capacity))
