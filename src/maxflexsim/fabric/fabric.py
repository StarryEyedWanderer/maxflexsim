...
class Fabric:
    ...
    def from_json(cls, path: str | Path) -> "Fabric":
        ...
        return cls(
            ...
            routing=RoutingParams(
                local=RoutingTier(
                    delay_per_hop=data["routing"]["local"]["delay_per_hop"],
                    capacity_bits_per_cycle=data["routing"]["local"].get(
                        "capacity_bits_per_cycle", default_capacity
                    ),
                ),
                regional=RoutingTier(
                    delay_per_hop=data["routing"]["regional"]["delay_per_hop"],
                    capacity_bits_per_cycle=data["routing"]["regional"].get(
                        "capacity_bits_per_cycle", default_capacity
                    ),
                ),
            ),
